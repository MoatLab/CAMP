...
def main(configs):
  ...
  outstanding_demand_rd = [get_vals("OFFCORE_REQUESTS_OUTSTANDING.DEMAND_DATA_RD", configs[i], csv_path) for i in range(len(configs))]

  lat = [outstanding_demand_rd[i]/demand_data_rd[i] for i in range(len(configs))]
  mlp = [outstanding_demand_rd[i]/outstanding_cyc_demand_rd[i] for i in range(len(configs))]
  aol = [lat[i]/mlp[i] for i in range(len(configs))]
  ...
  A, B = draw_scatter(workloads, ((pf_l1_any[0] - pf_l1_l3_hit[0])/pf_l1_any[0]) * ((fb_hit[0])/(fb_hit[0]+l1_miss[0]))*(l1_stall[0]-l2_stall[0])/cyc[0], l2_sd, output_path, "r-l2_sd", \
    "((pf_l1_any-pf_l1_l3_hit)/pf_l1_any)*(fb_hit)/(fb_hit+l1_miss))*(l1_stall-l2_stall)/cyc", "L2 slowdown", "", Linear)

  eq_cache = f"Cache Slowdown = {A:.2f} * ((pf_l1_any-pf_l1_l3_hit)/pf_l1_any)*(fb_hit)/(fb_hit+l1_miss))*(l1_stall-l2_stall)/cyc + {B:.2f}"
  print(eq_cache)

  indices = [workloads.index("lat1"), workloads.index("read2")]
  K = dram_sd / (l3_stall[0]/cyc[0])
  K_filtered = [K[i] for i in indices]
  aol_filtered = [aol[0][i] for i in indices]
  workloads_filtered = [workloads[i] for i in indices]
  A, B = draw_scatter(workloads_filtered, aol_filtered, K_filtered, output_path, "r-aol", \
    "aol", "K", "", Func1)
  demand_rd_metric = [Func1(aol[0], A, B)[i] * (l3_stall[0][i]/cyc[0][i]) for i in range(len(workloads))]
  C, D = draw_scatter(workloads, demand_rd_metric, dram_sd, output_path, "r-dram_sd", \
    "demand_rd_metric", "DRAM slowdown", "", Linear)

  eq_demand_rd = f"Demand Read Slowdown = 1/(({A:.2f} * mlp)/lat + {B:.2f}) * {C:.2f} * l3_stall/cyc + {D:.2f}"
  print(eq_demand_rd)

  A, B = draw_scatter(workloads, st_stall[0]/cyc[0], store_sd, output_path, "r-store_sd", \
    "store (BOUND_ON_STORES) cycles/cycles", "store slowdown", "", Linear)
  eq_store = f"Store Slowdown = {A:.2f} * store_stall/cyc + {B:.2f}"
  print(eq_store)

  with open("params.txt", "w") as f:
    f.write("\n".join([eq_cache, eq_demand_rd, eq_store]) + "\n")
