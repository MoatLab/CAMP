...
source "${THIS_SCRIPT_PATH}/paths.sh"
RUNDIR=$(echo "$(dirname "$PWD")")
REDIS_RUN_DIR="$RUNDIR/redis"
RSTDIR="${REDIS_RUN_DIR}/rst"
PERF_EVENTS_FILE="${REDIS_RUN_DIR}/perf_events.txt"
...
run_one_exp() {
  ...
  if [[ $et == "L100" ]]; then
    ...
    CPREFIX="numactl --cpunodebind 0 -i 0,1"
...
run_seq_cxl_interleave() {
  ...
  do
    y=$(expr $partitions - $x)
    echo $y > /sys/devices/system/node/node0/access0/il_weight
    echo $x > /sys/devices/system/node/node1/access0/il_weight
    for ((i = 0; i < ${#warr[@]}; i++)); do
      w=${warr[$i]}
      m=${marr[$i]}
      propid=${carr[$i]}
      run_one_exp "$w" "CXL-Interleave" "$y-$x" "$propid"
    done
  done
}
