...
GPT2_RUN_DIR="${RUNDIR}/gpt-2"
RSTDIR="${GPT2_RUN_DIR}/rst"
PERF_EVENTS_FILE="${GPT2_RUN_DIR}/perf_events.txt"
...
export GPT2_MDL_DIR="/mnt/sda4/gpt-2/models"
[[ ! -d "${GPT2_MDL_DIR}" ]] && echo "${GPT2_MDL_DIR} does not exist!" && exit
...
load_dataset()
{
  ...
  local id=$3
  local mdl=$4
  local MDL_DATASET="$mdl/model.ckpt.data-00000-of-00001"
  echo "    => Graph dataset: ${GPT2_MDL_DIR}/${MDL_DATASET}"
  if [[ ! -e "${GPT2_MDL_DIR}/${MDL_DATASET}" ]]; then
    echo "    => Error: Input graph ${GPT2_MDL_DIR}/${MDL_DATASET} not found .. skipping $w"
    exit
  fi
  echo "    => Loading graph into page cache first"
  if [[ $et == "L100" ]]; then
    numactl --membind 0 -- ${VMTOUCH} -f -t ${GPT2_MDL_DIR}/${MDL_DATASET} -m 64G
  elif [[ $et == "L0" ]]; then
    numactl --membind 1 -- ${VMTOUCH} -f -t ${GPT2_MDL_DIR}/${MDL_DATASET} -m 64G
  elif [[ $et == "CXL-Interleave" ]]; then
    numactl -i 0,1 -- ${VMTOUCH} -f -t ${GPT2_MDL_DIR}/${MDL_DATASET} -m 64G
  else
    ${VMTOUCH} -f -t ${GPT2_MDL_DIR}/${MDL_DATASET} -m 64G
  fi
  sleep 10
}
...
run_one_exp()
{
  ...
  if [[ $et == "L100" ]]; then
    ...
    run_cmd="numactl --cpunodebind 0 -i 0,1 -- ""${run_cmd}"
  else
...
run_seq_cxl_interleave()
{
  local type=$1
  check_cxl_conf
  local partitions=${INTERLEAVE_PARTITIONS:-10}
  local end=$(expr $partitions - 1)
  for ((i=1; i<=$end; i++))
  do
    j=$(expr $partitions - $i)
    echo $j > /sys/devices/system/node/node0/access0/il_weight
    echo $i > /sys/devices/system/node/node1/access0/il_weight
    for ((k = 0; k < ${#warr[@]}; k++)); do
      w=${warr[$k]}
      m=${marr[$k]}
      cd "$w" || { echo "ERROR: cannot cd into '$w', skipping"; continue; }
      run_one_exp "$w" "$type" "$j-$i" "$m"
      cd ../
    done
  done
  return
...
main()
{
  echo 1 > /sys/devices/system/node/node0/access0/il_weight
  echo 1 > /sys/devices/system/node/node1/access0/il_weight
  echo "Run INTERLEAVE ..."
  run_seq_cxl_interleave "CXL-Interleave"
  echo "Run LOCAL ..."
  run_seq "L100" "100"
  echo "Run REMOTE ..."
  run_seq "L0" "1"
